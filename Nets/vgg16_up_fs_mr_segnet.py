...
class VGG16_UP_FS_MR_SegNet(Autoencoder):
  def __init__(self, mc):
    ...
    # Setup mr labels
    self.aux_mr_labels = self._resize_labels(self.labels, \
                                             (45, 60), \
                                             'aux_mr_labels')
  ...
  def build(self):
    for step in xrange(self.seq_len):
      ...
      # Approx-Encoder
      encoder_out, aux_loss = self._approx_encoder(self.images_node[:, step, ...], \
                                                   step, lstm_reuse)
    ...
    with tf.variable_scope('cond_up', reuse=cond_up_reuse) as scope:
      if step == 0:
        ...
        encoder_out = self._deconv_layer(pool5, [2, 2, 512, 512], \
                                         ...
                                         "encoder_up")
        # f_shape = [4, 4, 512, 512] / stride = 2
        aux_up1 = self._deconv_layer(pool5, [2, 2, 512, 512], \
                                     [self.batch_size, 23, 30, 512], 2, \
                                     "aux_up1")
        aux_up2 = self._deconv_layer(pool5, [2, 2, 512, 512], \
                                     [self.batch_size, 45, 60, 512], 2, \
                                     "aux_up2")
        aux_mr_logits = self._conv_layer(aux_up2,
        ...
        aux_loss = self._loss(aux_mr_logits, self.aux_mr_labels[:, step, ...])
    
    return encoder_out, aux_loss
