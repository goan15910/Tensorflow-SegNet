...
class VGG16_LSTM_LR_SegNet(Autoencoder):
  def __init__(self, mc):
    ...
    self.down_ksize = 4
    self.down_stride = 4
    self.labels = self._resize_labels(self.labels, \
                                      self.down_ksize, \
                                      self.down_stride, \
                                      'lr_labels')
    ...
    for step in xrange(self.seq_len):
      ...
      with tf.variable_scope('image_resize', reuse=lstm_reuse) as scope:
        mr_images = self._max_pool(self.images_node[:, step, ...], \
                                   self.down_ksize, self.down_stride, name='lr_images')
  ...
  def _decoder(self, inputT, reuse):
    with tf.variable_scope('decoder', reuse=reuse) as scope:
      up5 = self._deconv_layer(inputT, [2, 2, 512, 512], [self.batch_size, 6, 8, 512], 2, "up5")
